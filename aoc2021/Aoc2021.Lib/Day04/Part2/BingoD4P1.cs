...
namespace Aoc2021.Lib.Day04.Part1
{
    public class BingoD4P2
    {
        ...
        public static BingoD4P2 Load(string lines)
        {
            var result = new BingoD4P2();
            ...
            foreach (var line in lines.LineByLine(true))
            {
                ...
                else
                {
                    ...
                    if (nbrOfLines == 5)
                    {
                        var board = BingoD4P2.Board.Load(result.Boards.Count+1, linesBoard.ToString());
        ...
        public Board LastBoardWithBingo { get; private set; }
        ...
        public class Board
        {
            ...
            public int Score => SumUnmarked * LastNumber;
        ...
        private List<Board> Draw()
        {
            var boardsWithBingo = new List<Board>();
            NbrOfNumbersDrawn++;
            ...
            foreach (var board in Boards)
            {
                ...
                {
                    board.Draw(number);
                    if (board.IsBingo) boardsWithBingo.Add(board);
                }
            }
        ...
        public void DrawNumbersTillLastOneWithBingo()
        {
            var nbrOfBoards = Boards.Count;
            var boardsWithBingo = new List<Board>();
            while (boardsWithBingo.Count != nbrOfBoards)
            {
                var newBoardsWithBingo = Draw();
                if (newBoardsWithBingo.Any())
                {
                    boardsWithBingo.AddRange(newBoardsWithBingo);
                    LastBoardWithBingo = newBoardsWithBingo.Last();
                }
            }
        }
